...
use App\Modules\Admin\ProviderWallets\Http\Requests\StoreProviderRequest;
...
class ProviderWalletController
{
    ...
    public function storeProvider(StoreProviderRequest $request): RedirectResponse
    {
        $provider = $this->walletService->createProvider($request->validated());

        return redirect()
            ->route('admin.provider-wallets.create', ['provider_id' => $provider->id])
